...
namespace IRTSearcher
{
    ...
    //    public Run LoadLibraryAndSearch(Run run, string iRTpath, double massTolerance)
    //    {
    ...
    //            //run.IRTPeaks = new List<IRTPeak>();
    //            //foreach(Library.Peptide peptide in irtLibrary.PeptideList)
    //            //{
    //            //    IRTPeak peak = new IRTPeak();
    //            //    peak.ExpectedRetentionTime = peptide.RetentionTime;
    //            //    peak.Mz = GetTheoreticalMz(peptide.Sequence, peptide.ChargeState);

    //            //    foreach (Library.Transition transition in irtLibrary.TransitionList)
    //            //    {
    //            //        if (Math.Abs(transition.PrecursorMz - peak.Mz) < 0.02)//chose this value as the smallest difference between two biognosis peptides is this
    //            //        {
    //            //            peak.AssociatedTransitions.Add(transition);
    //            //        }
    //            //    }
    ...
    //        Dictionary<string, Dictionary<string, List<SpectrumPoint>>> transitionSpectrumPoints = new Dictionary<string, Dictionary<string, List<SpectrumPoint>>>();
    ...
    //        //foreach (DictionaryEntry pde in irtLibrary.PeptideList)
    //        //{
    //        //    Library.Peptide peptide = (Library.Peptide)pde.Value;
    //        //    foreach (DictionaryEntry tde in irtLibrary.TransitionList)
    //        //    {
    //        //        Library.Transition transition = (Library.Transition)tde.Value;
    //        //        if (transition.PeptideId == peptide.Id)
    //        //        {
    //        //            var sps = new List<SpectrumPoint>();


    //        //            foreach (Scan scan in run.Ms2Scans)
    //        //            {
    //        //                var matchingpeaks = new List<SpectrumPoint>();

    //        //                if (scan.Spectrum != null)
    //        //                    matchingpeaks = scan.Spectrum.Where(x => Math.Abs(x.Mz - transition.ProductMz) < tolerance && x.Intensity > 100).ToList();

    //        //                sps = sps.Concat(matchingpeaks).ToList();
    //        //            }
    //        //            sps = sps.OrderByDescending(x => x.Intensity).ToList();
    //        //            if (!transitionSpectrumPoints.ContainsKey(peptide.Sequence))
    //        //            {
    //        //                transitionSpectrumPoints.Add(peptide.Sequence, new Dictionary<string, List<SpectrumPoint>>());
    //        //            }
    //        //            transitionSpectrumPoints[peptide.Sequence].Add(transition.Id, sps);
    //        //        }
    //        //    }

    //        //    for (int i = 0; i < 120; i++)
    //        //    {
    //        //        List<SpectrumPoint> lsp = new List<SpectrumPoint>();
    //        //        double d = i;
    //        //        foreach (var v in transitionSpectrumPoints[peptide.Sequence])
    //        //        {

    //        //            var o = v.Value.Where(x => Math.Abs(x.RetentionTime - d) < 0.1);
    //        //            if (o.Any())
    //        //                lsp.Add(o.First());
    ...
    //        //                transcount++;
    //        //        }
    //        //        if (lsp.Count == transcount)
    //        //        {
    //        //            foreach (var z in lsp)
    //        //                Console.Write(z.Intensity + " ");
    //        //            Console.WriteLine("");
    //        //            Console.WriteLine(lsp.First().RetentionTime);
    ...
    //        {
    //            logger.Error(ex, "The iRT file {0} was not able to be read - is it in use by another program?", iRTpath);
    //            throw;
    //        }
    //    }
